...
import { API_URL } from ".";
const ITEM_API = `${API_URL}/api/examinador/`;

export default class ExaminadorAPI {
    ...
    async update(id: number, nome: string, unidade: number, status: boolean, especialidade: string) {
        const response = await fetch(`${ITEM_API}${id}`, {
            method: 'PUT',
            body: JSON.stringify({ nome, unidade, status, especialidade }),
            headers: { 'Content-Type': 'application/json' }
        })
        return response.json();
    }
